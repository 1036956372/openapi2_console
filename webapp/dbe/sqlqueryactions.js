...
DBE.SQLQueryPanelActions = function(queryPanel) {
	...
	this.exec = new Ext.Action({
		...
		handler : function() {
			Ext.ux.SwfUploader.upload({
				upload_url : './dbeSQLQueryAction/execSqlFile.mvc',
				file_types : {
					type : '*.sql',
					desc : '所有文件'
				},
				upload_success_handler : function(file, data) {
					...
					var msg = '[' + file.name + ']文件执行';
					if (json.success) {
						msg += '成功:' + json.msg;
					} else {
						msg += '失败:' + json.msg;
					}
					queryPanel.runSQLFile(msg);
				}
			});
		}
	});
	...
	this.open = new Ext.Action({
		...
		handler : function() {
			Ext.ux.SwfUploader.upload({
				upload_url : './dbeSQLQueryAction/openSqlFile.mvc',
				file_types : {
					type : '*.sql',
					desc : '所有文件'
				},
	...
	this.save = new Ext.Action({
		...
		handler : function() {
			...
			window.location.href = './dbeSQLQueryAction/saveAsSQL.mvc?sql='
					+ sql;
		}
	});
