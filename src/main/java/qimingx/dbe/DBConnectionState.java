...
import qimingx.dbe.service.DBInfoService;
import qimingx.dbe.service.impl.DefaultDBInfoService;
import qimingx.utils.MyUtils;
import qimingx.utils.SQLUtils;
...
//	private static final String KEY_CURRENT_STATE = "cn.com.qimingx.dbe.DBConnectionState.KEY";
	...
	//想要在用户登录时引用这个景太常亮，暂时把它改为public
	public static final String KEY_CURRENT_STATE = "cn.com.qimingx.dbe.DBConnectionState.KEY";
	...
	public String toString() {
		return "DBConnectionState [dbConnection=" + dbConnection + ", dbType=" + dbType + ", service=" + service + "]";
	}

	/**
	 * 隐藏构建器
	 */
	public DBConnectionState(Connection conn, DBTypeInfo dbtype) {
		dbConnection = conn;
		this.dbType = dbtype;
	}
	...
	/**
	 * DBExplor连接当前平台登录的数据库账户
	 * @param typeName	数据库类型
	 * @param conn	当前数据库连接对象
	 * @param sess
	 * @return	返回操作结果
	 * */
	public static ProcessResult<String> connect(String typeName,Connection conn,HttpSession sess){
