...
import qimingx.dbe.action.bean.GridQueryLoadBean;
import qimingx.dbe.service.DBInfoService;
import qimingx.json.MyJSONUtils;
...
public class SQLQueryOperator {
	...
	public ProcessResult<JSON> execute(DBInfoService service,
		...
		String sqlText = param.getSql();
		sqlText = sqlText == null ? "" : sqlText;
		if (sqlText.length() > 0) {
			if (sqlText.startsWith("SELECT") || sqlText.startsWith("select")) {
				return executeQuery(sqlText, service, param);
			} else {
				return executeUpdate(sqlText, service, param);
			}
		} else {
