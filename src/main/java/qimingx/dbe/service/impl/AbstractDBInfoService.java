package qimingx.dbe.service.impl;
...
import qimingx.dbe.FieldDataType;
import qimingx.dbe.LobObject;
import qimingx.dbe.TableColumnInfo;
import qimingx.dbe.TableDataInfo;
import qimingx.dbe.TableInfo;
...
import qimingx.utils.sql.SQLExec;
...
public abstract class AbstractDBInfoService implements DBInfoService {
	...
	public ProcessResult<String> executeByFile(File file) {
		...
		try {
			...
			pr.setSuccess(true);
			pr.setData(sqlexec.getResult());
			pr.setMessage(pr.getData());
			return pr;
		} catch (Exception e) {
			log.error("执行数据脚本出错" + e.getMessage());
