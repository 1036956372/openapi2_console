...
public class ArmViewportTag extends TagSupport {
	...
	private String getPanelScript() {
		...
		//dto2将userid传递给tagSupportService.getCardList，返回一个cardList
		...
		String dtoStr = dto.toJson(G4Constants.FORMAT_Time);		//debug用
		StringWriter writer = engine.mergeTemplate(template, dto);
		String treesString = generateCardTrees(dto);			//卡片树
	...
	private String generateCardTrees(Dto pDto) {
		...
		for (int i = 0; i < cardList.size(); i++) {
			...
			Dto dto = new BaseDto();
			dto.put("menuList", generateMenuPathName(menuList, pathDto));				//生成菜单路径对应中文名
